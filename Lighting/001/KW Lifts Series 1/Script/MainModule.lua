	...
	start = function(ref,cfg)		
		...
		local function weldRecursive(obj)
			for _,v in pairs(obj:GetChildren()) do
				if v.ClassName == "Part" or v.ClassName == "WedgePart" or v.ClassName == "CornerWedgePart" or v.ClassName == "UnionOperation" or v.ClassName == "MeshPart" or v.ClassName == "TrussPart" or v.ClassName == "Seat" then
					weld(v)
				end
		...
		local function voiceRoutine()
			wait()
			if cfg.CustomVoice then
			if cfg["F"..floor] ~= nil and cfg.StandClear then
				voice.SoundId = cfg["F"..floor]
				voice:Play()
				wait(cfg["F"..floor.."Delay"])
				voice.SoundId = cfg.StDoorVoice
				voice:Play()
				wait(cfg.StDoorDelay)
			elseif cfg["F"..floor] ~= nil  and cfg.StandClear == false then
				voice.SoundId = cfg["F"..floor]
				voice:Play()
				wait(cfg["F"..floor.."Delay"])
			elseif cfg["F"..floor] == nil  and cfg.StandClear then
				voice.SoundId = cfg.StDoorVoice
				voice:Play()
				wait(cfg.StDoorDelay)
			else return end
			...
			if cfg.DirSoundWhenMove == false then
			if direction == 1 then
				voice.SoundId = cfg.UpVoice
				voice:Play()
			elseif direction == -1 then
				voice.SoundId = cfg.DownVoice
				voice:Play()
			end
			end
			else
		...
		local function openDoors(bypass)
			...
			if (nudgeCounter > cfg.reopenLimit or (mode ~= "auto" and mode ~= "priority")) and not quickClose then
				if cfg.DoorClosingSound == true and cfg.ClosingSoundID ~= nil then
					voice.SoundId = cfg.ClosingSoundID
					voice:Play()
				end
				if shaft[tostring(floor)]:FindFirstChild("FrontDoor") and car:FindFirstChild("FrontDoor") then
					for i,v in pairs(shaft[tostring(floor)].FrontDoor:GetChildren()) do
		...
		if car:FindFirstChild("Controls") then
			car.Controls.Switch.ClickDetector.MouseClick:connect(function()
				if mode ~= "manual" then
					...
					inService = false
					mode = "manual"
					if cfg.narrator and cfg.CustomVoice == false then
		...
		spawn(function()
			while true do
				...
				if nearFloor ~= floor and cfg.FloorPassChime then
				...
				--/ Call Management /--
				if (not doorState or cfg.idleHold) and motor == 0 and mode == "auto" then
					spawn(handleCalls)
					if floor == topFloor or bottomFloor then
						direction = 0
					end
				end
